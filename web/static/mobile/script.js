...
function m_toggle_reply(index) {
	...
	if (req.readyState == 4 /* complete */) {
	    ...
	    reply_div.innerHTML += '<input type="button" value="确定" class="reply-submit" onclick="m_do_reply(\'' + index + '\', \''+data_recv['_xsrf']+'\');" />';
...
function m_subject_reply(index) //subject index,start from 0
{
    ...
    req.onreadystatechange = function() {
        if(req.readyState == 4) {
            ...
            if (data_recv['success']) {
            reply_div.innerHTML = '<textarea rows="5" id="reply-content-' + index + '">\n\n\n' + data_recv['content'] + '</textarea>';
            reply_div.innerHTML += '<input type="button" value="确定" class="reply-submit" onclick="m_do_subject_reply(\'' + index + '\', \''+data_recv['_xsrf']+'\');" />';
            } else{
